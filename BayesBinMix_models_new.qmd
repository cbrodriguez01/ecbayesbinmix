...
res_acs15<-res_all$res_acs15_10
res_acs19<-res_all$res_acs19_10
```
...
agedist<-load("/Users/carmenrodriguez/Desktop/Research Projects/Census Data/census_age_dist.RData")
...
#to describe patterns
dat_15 %>% group_by(cluster) %>% arrange(desc(theta_kj),.by_group = T)

dat_15_1<-dat_15 %>%  mutate(hml= case_when(
                             theta_kj >=0.70 ~ 1,
                             theta_kj< 0.70 & theta_kj >=0.4 ~ 2,
                             theta_kj < 0.4 ~ 3,))

dat_15_1$hml <- factor(dat_15_1$hml, levels = 1:3, labels = c("High", "Med", "Low"))


out1<-dat_15_1 %>%  group_by(cluster) %>% arrange(desc(theta_kj),.by_group = T) %>% select( cluster, NSES_VARS, theta_kj, hml)
...
#to describe patterns
#dat_19 %>% group_by(cluster) %>% arrange(theta_kj,.by_group = T)
...
classificationprobs_19$ClusterAssignment<- factor(classificationprobs_19$ClusterAssignment, levels = 1:9, labels = paste0("cluster", 1:9, sep= " "))

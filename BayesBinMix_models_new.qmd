...
#Output probabilities
classificationprobs_19 %>% filter(ClusterAssignment_ecr == 5) %>% select(ClusterAssignment_ecr, temp, maxProb, maxProb2)
...
table(acs15=dat15$cluster_color, acs10=dat10$cluster_color)
table(acs10=dat10$cluster_color, acs19=dat19$cluster_color)
table(acs15=dat15$cluster_color, acs19=dat19$cluster_color)

#We are going to use 2006-2010 as the reference clusters, that is i am looking at concondance between 2010-> 2015 and 2010->2019

dat10_1<- dat10[, c("GEOID", "cluster_color")]
dat10_1 <- dat10_1 %>% rename(cluster_color_10= cluster_color)
dat1015<-merge(dat15[, c("GEOID", "cluster_color")], dat10_1, by= "GEOID")

dat1015$cluster_color_15<- ifelse(dat1015$cluster_color_10 %in% dat1015$cluster_color, dat1015$cluster_color_10, dat1015$cluster_color)

dat1015$cluster_color_15<-factor(dat1015$cluster_color_15,levels = 1:8, labels= c("red", "blue","green","purple","orange","yellow", "brown","pink"))


check<-dat1015 %>% filter(cluster_color_10 %in% cluster_color) %>% select(cluster_color_10, cluster_color,cluster_color_15)

table1::table1( ~ cluster_color_10|cluster_color, data = dat1015)


dat1015$cluster_col15<- factor(dat1015$cluster_color, levels = dat1015[order(dat1015$cluster_color_10), "cluster_color"])
levels(dat1015$cluster_col15)




#Identify concordant observations: Determine which observations have the same labels in both variables despite having different distributions. You can achieve this by comparing the two variables.

#Create a new variable: Once you have identified the concordant observations, create a new variable that matches the labeling based on one of the variables.

dat1015 %>% filter(cluster_color == "red") %>% select(GEOID, cluster_color, cluster_color_10)

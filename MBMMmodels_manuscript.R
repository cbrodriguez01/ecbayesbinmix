...
print(res19_poi) 
#Small clusters that need more exploring: 2,4,5,7-- i will probs reclassify
...
#only re-assign cluster 5 as it is the smallest--even the highest probs are not all >0.8
...
#Prep data
classificationprobs_poi$GEOID<-row.names(censusdata_bin$acs5_2019_bin)
classificationprobs_poi$ClusterAssignment_recoded<- factor(classificationprobs_poi$cluster_reassingment_recode,
...
head(classificationprobs_poi %>% filter(cluster_reassingment_recode ==5) %>% select(cluster_reassingment_recode, maxProb, smaxProb,maxProb_updated),20)



dat19_clusters_poi<- classificationprobs_poi %>% select(GEOID,ClusterAssignment_recoded, maxProb_updated)

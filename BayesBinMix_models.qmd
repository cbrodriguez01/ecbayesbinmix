...
classificationprobs_10$CT<-row.names(classificationprobs_10) #I will add actual tracts later
...
datplot<- classificationprobs_10 %>% select(CT,ClusterAssignment, maxProb)


datplot%>%  ggplot(aes(x = ClusterAssignment, y = maxProb, fill = ClusterAssignment)) +
  geom_boxplot() +
  labs(fill = "Cluster Assignment", y = "Classification Probability after ECR algorithm", x= "")

dattable<-datplot %>% rename(`Average Assignment Probability (ECR Algorithm)` = maxProb,
...
#Prep Data for boxplot
classificationprobs_15$CT<-row.names(classificationprobs_15) #I will add actual tracts later

classificationprobs_15$ClusterAssignment<- factor(classificationprobs_15$ClusterAssignment, levels = 1:4, labels = c("cluster 1: 0.2956", "cluster 2: 0.2960", "cluster 3: 0.2044", "cluster 4: 0.2038"))

datplot15<- classificationprobs_15 %>% select(CT,ClusterAssignment, maxProb)


datplot15%>%  ggplot(aes(x = ClusterAssignment, y = maxProb, fill = ClusterAssignment)) +
  geom_boxplot() +
  labs(fill = "Cluster Assignment", y = "Classification Probability after ECR algorithm", x= "")

dattable15<-datplot15 %>% rename(`Average Assignment Probability (ECR Algorithm)` = maxProb,
                    `Cluster Assignment` = ClusterAssignment) 

table1::table1(~`Average Assignment Probability (ECR Algorithm)` |`Cluster Assignment`, data= dattable15, overall = F)

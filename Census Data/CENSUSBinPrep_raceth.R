...
acs5_2010_bin<- as.data.frame(sapply(2:length(acs5_2010_wide2), function(i) {ifelse(is.na(acs5_2010_wide2[,i]), NA, ifelse(as.numeric(acs5_2010_wide2[,i]) >= as.numeric(acs_2010_medians[i-1,2]),1,0))}))
...
#Create the binary variables, 1 if above or equal to median for MA and 0 if below (vars 2:20)  and then we append lack of p.
acs5_2015_bin<- as.data.frame(sapply(2:length(acs5_2015_wide2), function(i) {ifelse(is.na(acs5_2015_wide2[,i]), NA, ifelse(as.numeric(acs5_2015_wide2[,i]) >= as.numeric(acs_2015_medians[i-1,2]),1,0))}))
...
acs5_2019_wide1 <- acs5_2019_wide  %>% select( !c( "Femalehousehold_2019_E","SNAP_2019_E","Two_or_more_rooms_2019_E",  
                                                   ...
                                                   "Two_or_more_rooms_2019_P", "Total_housing_2019",    "NH_some_other_race_2019",
                                                   "NH_two_or_more_races_2019",  "NonHispanicWhite_2019" ,  "minoritystatus_2019" ))
...
#Create the binary variables, 1 if above or equal to median for MA and 0 if below (vars 2:20)  and then we append lack of p.
acs5_2019_bin<- as.data.frame(sapply(2:length(acs5_2019_wide2), function(i) {ifelse(is.na(acs5_2019_wide2[,i]), NA, ifelse(as.numeric(acs5_2019_wide2[,i]) >= as.numeric(acs_2019_medians[i-1,2]),1,0))}))
...
saveRDS(dats, "/Users/carmenrodriguez/Desktop/Research Projects/Census Data/censusdata_bin_raceeth_042524.rds")
...
censusdata_bin <- readRDS("/Users/carmenrodriguez/Desktop/Research Projects/Census Data/censusdata_bin_raceeth_042524.rds")
...
colnames(acs10)<- c("medianincome"  , "Femalehousehold" ,"Education9years",        
                    ...
                    "OwnerOccupiedUnitP" , "RenterOccupiedUnitP" , "No_vehicle", "hispanic", "NHB", "NHA",
                    "crowded_housing", "Belowpl", "working_class", "Lackplumbing") 
...
tab<-table1::table1( ~ Lackplumbing + medianincome + BachelorHigherP +        
                       ...
                       SNAP + working_class + Belowpl + crowded_housing + hispanic + NHB+ NHA + ENProficiency  | survey_year_fct, data = allwaves, overall = F  )
library(flextable)
t1flex(tab) %>% 
  save_as_docx(path="/Users/carmenrodriguez/Desktop/Research Projects/Census Data/censusbindist_042524.docx")
...
tab1<-table1::table1( ~ Lackplumbing + medianincome + BachelorHigherP +        
                        Education9years + Femalehousehold + HighSchoolHigherP +
                        No_vehicle + OwnerOccupiedUnitP + RenterOccupiedUnitP + UnemployementP +         
                        SNAP + working_class + Belowpl + crowded_housing +  hispanic + NHB+ NHA + ENProficiency  | survey_year_fct, data = allwaves, overall = F, extra.col=list(`P-value`=pvalue))
tab1


###########################################################################
## Subset data to not inlude race/ethnicity variables
#d = 15
...
acs10<-acs5_2010_bin
colnames(acs10)
acs10_1 <- acs10 %>% select(!c("NonHispanicBlack_2010","NonHispanicAsian_2010", "Hispanic_or_Latino_2010"))
names(acs10_1)

acs15<-acs5_2015_bin
colnames(acs15)
acs15_1 <- acs15 %>% select(!c("NonHispanicBlack_2015","NonHispanicAsian_2015", "Hispanic_or_Latino_2015"))
names(acs15_1)

acs19<-acs5_2019_bin
colnames(acs19)
acs19_1 <- acs19 %>% select(!c("NonHispanicBlack_2019","NonHispanicAsian_2019", "Hispanic_or_Latino_2019"))
names(acs19_1)

## Save binary dataset
dats1<-list(acs10_1, acs15_1, acs19_1)
saveRDS(dats1, "/Users/carmenrodriguez/Desktop/Research Projects/Census Data/censusdata_bin_noraceeth_061024.rds")
